<template>
    <div>
        <div v-if="focus" @click="focus=false" class="bg-black opacity-10 absolute right-0 left-0 top-0 bottom-0"></div>
        <div class="relative z-10">
            ...
            <input @focus="focus=true" class="w-64 mr-6 bg-gray-200 border focus:outline-none focus:border-blue-500 focus:shadow focus:bg-gray-100 border-gray-400 pl-8 rounded-full pr-3 py-1 text-sm" @input="search" type="text" placeholder="Search..." id="searchTerm" v-model="searchTerm">

            <div v-if="focus" class="absolute bg-blue-900 text-white rounded-lg p-4 w-96 right-0 mr-6 mt-2 shadow z-20">
                <div v-if="results.length === 0">
                    No results found for "{{ searchTerm }}"
                </div>
                <div v-else v-for="{ data } in results" @click="focus=false">
                    <router-link class="py-2 block" :to="{ name: 'ContactShow', params: { id: data.contact_id }}">
